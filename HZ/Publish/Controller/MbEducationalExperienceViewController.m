    ...
    if(indexPath.row==0){
        ...
        if (self.endTime==nil) {
         ...
         cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
            ...
            self.schoolField = [[UITextField alloc]initWithFrame:CGRectMake(90, 0, viewWidth - 125, 44)];
        ...
        //输入框
        self.professional = [[UITextField alloc]initWithFrame:CGRectMake(90, 22-size.height/2, viewWidth - 125, size.height)];
...
-(void)initPickView{
    //弹出视图
    self.pickerView = [[MbPickerView alloc]initWithFrame:CGRectMake(0, viewHeight, viewWidth, 326)];
...
{
    if (viewHeight==480&&[self.professional isEditing]) {
        CGRect frame = textField.frame;
        int offset = frame.origin.y + 32 - (viewHeight - 494.0);//键盘高度216
